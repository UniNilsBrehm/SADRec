...
def setup_device_selection(self):
    ...
    self.layout.addWidget(QLabel("Input Device:"))
    self.layout.addWidget(self.input_device_combo)
    self.layout.addWidget(QLabel("Output Device:"))
    self.layout.addWidget(self.output_device_combo)

    self.populate_audio_devices()
    self.input_device_combo.currentIndexChanged.connect(self.update_audio_devices)
    self.output_device_combo.currentIndexChanged.connect(self.update_audio_devices)
...
def populate_audio_devices(self):
    ...
    for i in range(device_count):
        ...
        if info['maxOutputChannels'] > 0:
            ...
            self.output_device_combo.addItem(name)

def update_audio_devices(self):
    input_index = self.input_devices[self.input_device_combo.currentIndex()][1]
    output_index = self.output_devices[self.output_device_combo.currentIndex()][1]
    self.recorder.change_input_output_devices(input_index, output_index)